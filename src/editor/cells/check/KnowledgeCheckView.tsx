...
export function KnowledgeCheckView({
  ...
  // Participate in the trace: a named check writes `$[name]`, so it's a graph
  // node. `passed` is the published signal (pulse when it flips).
  const trace = useCellTrace(id, passed);
    ...
    <NodeViewWrapper
      ...
      {!reading && (
        <div className="flex items-center gap-2.5 border-b border-reactive-border bg-reactive-bg px-3 py-2">
          ...
          <span className="font-mono! inline-flex items-center gap-1.5 rounded-lg border border-transparent bg-black-a2 dark:bg-white-a2 px-2.5 py-0.5 text-[12.5px] font-medium text-text">
      ...
      {!reading && (
        <div className="font-mono! flex items-stretch gap-0 overflow-x-auto border-t border-border-subtle bg-gray-2 px-4 py-3">
  ...
  return (
    <div className="flex flex-wrap items-center gap-3">
      ...
      {shown && !passed && correctText != null && correctText !== "" && (
        <span className="inline-flex items-center gap-1.5 rounded-lg border border-brand-5 bg-brand-4 px-2.5 py-1.5 font-mono text-[13px] text-brand-11">
